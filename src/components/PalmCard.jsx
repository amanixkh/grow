...
const LEVEL_COLOR = { high: "var(--accent)", med: "var(--accent-amber)", low: "var(--accent-red)" };
...
export default function PalmCard({ palm, isActive, isDimmed, onToggle }) {
  ...
  return (
    <div
      ref={cardRef}
      onClick={onToggle}
      className={`palm-card-item group relative bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 flex flex-col gap-4 cursor-pointer
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex gap-3.5 items-start min-w-0">
          <div className="w-12 h-12 rounded-[12px] flex-shrink-0 flex items-center justify-center bg-gradient-to-br from-[var(--tag-green-bg)] to-[var(--tag-tan-bg)]">
            <svg viewBox="0 0 24 24" fill="none" className="w-[24px] h-[24px] text-[var(--accent)]">
          ...
          <div className="min-w-0">
            <h3 className="text-[19px] font-extrabold text-[var(--heading)] truncate">{palm.name}</h3>
          </div>
        </div>
      ...
      <p className="text-[13.5px] text-[var(--muted)] leading-relaxed min-h-[42px]">{palm.desc}</p>

      <div className="flex flex-col gap-3 py-1">
        <div className="flex items-center gap-3">
          <div className="text-[12px] text-[var(--muted)] font-bold w-[92px] flex-shrink-0">Salinity</div>
          <LevelMeter level={palm.salinity} />
        </div>
        <div className="flex items-center gap-3">
          <div className="text-[12px] text-[var(--muted)] font-bold w-[92px] flex-shrink-0">Water needs</div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {palm.areas.map((a) => (
          <span className="text-[11px] font-bold text-[var(--tag-green-text)] bg-[var(--tag-green-bg)] px-2.5 py-1 rounded-full" key={a}>{a}</span>
        ))}
      </div>
      ...
      <div className="flex gap-2.5 items-start text-[12.5px] text-[var(--muted)] bg-[var(--surface-soft)] border border-[var(--border)] rounded-[11px] px-3.5 py-3 leading-relaxed">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3.5 h-3.5 flex-shrink-0 mt-0.5 text-[var(--accent-amber)]">
