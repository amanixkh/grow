...
export default function Nakheel() {
  ...
  const filteredPalms = useMemo(() => {
    ...
    let list = PALMS.filter((palm) => {
      const matchesQuery =
        !q || palm.name.toLowerCase().includes(q);

      const matchesRegion =
        !filters.region ||
        palm.areas.includes(filters.region);

      const matchesSalinity =
        !filters.salinity ||
        palm.salinity === filters.salinity;

      const matchesWater =
        !filters.water ||
        palm.water === filters.water;
    ...
    if (filters.sort === "salinity") {
      list.sort(
        (a, b) =>
          LEVEL_RANK[b.salinity] -
          LEVEL_RANK[a.salinity]
      );
    }
